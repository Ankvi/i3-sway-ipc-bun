...
export async function toggle(terminalAppId = "alacritty-dropdown") {
    ...
    if (!dropdownTerminal) {
        ...
        log.error(JSON.stringify(content.map((x) => ({
            id: x.id,
            app_id: x.app_id,
            name: x.name,
        })), null, 4));
    ...
    if (!(dropdownTerminal.rect.width === newWidth && dropdownTerminal.rect.height === newHeight)) {
        log.info(`Updating terminal size. Old: ${dropdownTerminal.rect.width}x${dropdownTerminal.rect.height}. New: ${newWidth}x${newHeight}`);
