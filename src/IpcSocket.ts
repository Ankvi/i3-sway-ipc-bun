...
import { IpcEvent, SocketEvent, type SocketEvents, type WindowEvent } from "./types/events";
...
export class IpcSocket extends EventEmitter<SocketEvents> {
	...
	static async getSocket(): Promise<IpcSocket> {
		...
		if (!socketPath) {
			...
			socketPath = (await new Response(getSocketPathProc.stdout).text()).trim();
		}
	...
	public close(signal: NodeJS.Signals): void {
	...
	private _processMessage() {
		...
		while (bytesRead < readableLength) {
			try {
				const header = message.subarray(bytesRead, bytesRead + HEADER_LENGTH);
				...
				const payload = message.subarray(bytesRead, bytesRead + payloadLength);
				...
				logger.debug(`bytesRead: ${bytesRead}, socket.readableLength: ${readableLength}`);
				logger.debug("---------------------");
			}
		}
	}
	...
	private _handleWindowEvent(event: WindowEvent) {
		...
		switch (event.change) {
			...
			default: {
				logger.warn("Received unknown window event change:", event.change);
			}
		}
	}
